...
public class AnonymizeDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		//reduce tasks number set 10
		job.setNumReduceTasks(10);//maximum reduce task

		TextInputFormat.setInputPaths(job, new Path(otherArgs[0]));
		TextOutputFormat.setOutputPath(job, new Path(otherArgs[1]));

		job.setOutputKeyClass(IntWritable.class); //hadoop 0 version vs hadoop 1 version difference point.
		job.setOutputValueClass(Text.class); // It is mean that job for mapper. if you want transfer other data type,
		//you must set Mapper outputkeyclass(e.g job.setMapOutputKeyClass(IntWritable.class))
