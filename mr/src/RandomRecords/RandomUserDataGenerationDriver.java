...
public class RandomUserDataGenerationDriver {
		...
		public static class RandomStackoverflowRecordReader extends
			...
			public boolean nextKeyValue() throws IOException,
				...
				if (createdRecords < numRecordsToCreate) {
					...
					int Reputation = Math.abs(rndm.nextInt()) % 1000000000;
					String CreationDate = frmt.format(Math.abs(rndm.nextLong()));
					String DisplayName = getDisplayText();
					int EmailHash = String.valueOf(Math.abs(rndm.nextInt()) % 1000000000).hashCode();
					String LastAccessDate = frmt.format(Math.abs(rndm.nextLong()));
					String Location = getCity();
					String AboutMe = getRandomText();
					int Views = Math.abs(rndm.nextInt()) % 1000000;
					int UpVotes = Math.abs(rndm.nextInt()) % 1000;
					int DownVotes = Math.abs(rndm.nextInt()) % 1000;
					// Create a string of text from the random words
					String randomRecord = "<row Id=\"" + rowId + "\" Reputation=\""
							+ Reputation + "\" CreationDate=\"" + CreationDate + "\" DisplayName=\""
							+ DisplayName + "\" EmailHash=\"" + EmailHash
							+ "\" LastAccessDate=\"" + LastAccessDate + "\" Location=\"" + Location
							+ "\" AboutMe=\"" + AboutMe + "\" Views=\"" + Views
			...
			private String getCity() {
				int numWords = Math.abs(rndm.nextInt()) % cityWords.size();

				return cityWords.get(numWords);
			}
			...
			private String getDisplayText() {
				...
				for (int i = 0; i < numWords; ++i) {
					bldr.append((char)(Math.abs(rndm.nextInt()) % ('z' - 'a') + 'a'));
				}
				return bldr.toString();
			}
	...
	public static void main(String[] args) throws Exception {
		...
		Path cityList = new Path(otherArgs[3]);
		Path outputDir = new Path(otherArgs[4]);
		Job job = new Job(conf, "RandomDateGenerationDriver");
		job.setJarByClass(RandomUserDataGenerationDriver.class);
		...
		RandomStackOverflowInputFormat.setRandomWordList(job, cityList);
